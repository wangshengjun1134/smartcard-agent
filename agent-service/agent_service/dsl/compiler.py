...
class APDSLExecutor:
    ...
    async def _execute_command(
        ...
        """
        from agent_service.apdu.builders.select_builder import build_select_file
        from agent_service.apdu.builders.read_builder import build_read_binary, build_read_record
        from agent_service.apdu.builders.auth_builder import build_verify_pin
    ...
    def _parse_data(self, data: bytes, format_name: str) -> Any:
        ...
        if format_name == "imsi_bcd":
            from agent_service.skills.composite.read_imsi import parse_imsi
            return parse_imsi(data)

        elif format_name == "iccid_bcd":
            from agent_service.skills.composite.read_iccid import parse_iccid
