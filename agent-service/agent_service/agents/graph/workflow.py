...
from agent_service.agents.core.message import build_user_message, build_assistant_message, build_system_message
from agent_service.agents.core.tool_scheduler import ToolScheduler
from agent_service.agents.tools.builtin import register_builtin_tools
from agent_service.agents.core.events import emit_thinking, emit_content
...
**重要：连接状态处理**
- 如果工具返回 "Not connected to card" 或类似连接错误，这是**用户需要操作的问题**，不是你能通过发送更多 APDU 解决的。
- 遇到连接错误时，请**直接告诉用户需要先连接卡片**，然后停止尝试，不要继续发送 APDU 命令。
- 你无法控制硬件连接状态，只能发送指令。连接问题需要用户在前端手动操作。
...
async def run_agent_core(
    ...
    if session_id:
        try:
            from agent_service.api.session import get_session_messages
...
def _update_assistant_message(
    ...
    """
    try:
        from agent_service.utils.database import get_session_db_connection
